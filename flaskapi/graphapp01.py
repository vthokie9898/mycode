...
def sshlogin(ip, un, passw):
    ...
    ssh_stdin, ssh_stdout, ssh_stderr = sshsession.exec_command("uptime -p")
    sshresult = ssh_stdout.read().decode('utf-8')
    with open("sshresult", "w") as myfile:
        myfile.write(sshresult)
    timematch = re.search(r"^up\s(\d+)\sdays,\s(\d+)\shours,\s(\d+)\sminutes", sshresult)
    mins = 0
    days = re.search(r"(\d+)\sdays", sshresult)
    if days:
        mins += int(days[1]) * 1440

    hours = re.search(r"(\d+)\shours", sshresult)
    if hours:
        mins += int(hours[1]) * 60
